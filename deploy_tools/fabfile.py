...
REPO_URL = 'git@github.com:SergiySavarin/tdd_django.git'
...
def _update_settings(source_folder, site_name):
    ...
    sekret_key_file = source_folder + '/mysite/secret_key.py'
    if not exists(sekret_key_file):
        chars = 'abcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*(-_=+)'
        key = ''.join(random.SystemRandom().choice(chars) for _ in range(50))
        append(sekret_key_file, "SECRET_KEY = '%s'" % (key,))
